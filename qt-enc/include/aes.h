#pragma once

#include <iostream>
#include <vector>
#include <cstdint>
#include <iomanip>
#include <sstream>
#include <string>

#include "API.h"

using namespace std;

class EXPORT_API AES128 {
public:
    AES128(const vector<uint8_t>& key);

    void EncryptBlock(vector<uint8_t>& block);
    void DecryptBlock(vector<uint8_t>& block);

    vector<uint8_t> EncryptMessage(const string& message);
    string DecryptMessage(const vector<uint8_t>& encryptedMessage);

private:
    vector<vector<uint8_t>> RoundKeys;

    const vector<uint8_t> SBox = {
        // S-box table
        
    };

    const vector<uint8_t> InvSBox = {
        // Inverse S-box table
        
    };

    const vector<uint8_t> Rcon = {
        0x01, 0x02, 0x04, 0x08,
        0x10, 0x20, 0x40, 0x80,
        0x1B, 0x36, 0x6C, 0xD8,
        0xAB, 0x4D, 0x9A, 0x2F
    };

    void KeyExpansion(const vector<uint8_t>& key);
    void AddRoundKey(vector<uint8_t>& state, int round);

    void SubBytes(vector<uint8_t>& state);
    void InvSubBytes(vector<uint8_t>& state);

    void ShiftRows(vector<uint8_t>& state);
    void InvShiftRows(vector<uint8_t>& state);

    void MixColumns(vector<uint8_t>& state);
    void InvMixColumns(vector<uint8_t>& state);

    uint8_t Multiply(uint8_t x, uint8_t y);

    void RotWord(vector<uint8_t>& word);
    void SubWord(vector<uint8_t>& word);

    vector<uint8_t> PadMessage(const string& message);
    string UnpadMessage(const vector<uint8_t>& message);
};

